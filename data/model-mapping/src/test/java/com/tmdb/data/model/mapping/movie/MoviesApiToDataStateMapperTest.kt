...
class MoviesApiToDataStateMapperTest {
    ...
    fun `mapping Success ApiResponse_Success to DataState_Success`() {
        val input: ApiResponse<DataPage<Movie>, NetworkErrorModel> =
            ApiResponse.Success(DataPage(page = 0, results = listOf(com.tmdb.data.model.mapping.util.ModelUtil.movieModel)))

        val actual = listMapper.invoke(input)
        val expected = DataState.Success(listOf(com.tmdb.data.model.mapping.util.ModelUtil.movieDataModel))
