...
fun <D> getDependenciesProviderFromContext(context: Context): DependenciesProvider<D> = getDependenciesProviderFromContextInternal(context)

private inline fun <T, reified D : DependenciesProvider<T>> getDependenciesProviderFromContextInternal(
    context: Context
): DependenciesProvider<T> {
    return (context as? D) ?: throw IllegalStateException("Please register provider for ${D::class.java} dependency")
}